...
import me.chenqiang.pdf.common.attribute.ColorMap;
...
public class WatermarkMaker implements IEventHandler {
	...
	protected WatermarkMaker(WatermarkMaker origin) {
		this.renderers = new ArrayList<>(origin.renderers);
	}
	...
	public static void renderImage(PdfPage page, PdfCanvas pdfCanvas, ImageData image,
		...
		if (setting != null) {
			...
			if(setting.width > 0) {
				calculatedWidth = setting.width;
			}
			else if(setting.width == 0) {
				calculatedWidth = image.getWidth();
			}
			else {
				calculatedWidth = -1 * setting.width * pageSize.getWidth();
			}

			float posX = setting.offsetX - 1 * calculatedWidth / 2;
			float posY = setting.offsetY - 1 * calculatedWidth * image.getHeight() / (2 * image.getWidth());

			pdfCanvas.addImage(image, posX, posY, calculatedWidth, false);
	...
	public static void renderText(PdfDocument pdfDoc, PdfPage page, PdfCanvas pdfCanvas, String text,
		...
		canvas.setProperty(Property.FONT, setting.font.getFont());
